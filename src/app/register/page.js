...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      await updateProfile(result.user, { displayName: form.name.value, photoURL: form.photoURL.value });
    ...
    <section className="container flex min-h-[80vh] items-center justify-center py-12">
      <div className="w-full max-w-md rounded-2xl border border-[var(--line)] bg-white p-8 shadow-lg">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-[var(--muted)]">Join AxleWay and start renting today</p>
        </div>
        ...
        <div className="my-6 flex items-center gap-3">
          ...
          <span className="text-xs text-[var(--muted)]">or register with email</span>
          <div className="h-px flex-1 bg-[var(--line)]" />
        </div>
        ...
        <form className="space-y-4" onSubmit={handleRegister}>
          <label className="block text-sm font-bold">
            Full Name
            <input required autoComplete="name" className={inputClass} name="name" placeholder="Your full name" type="text" />
          </label>
          <label className="block text-sm font-bold">
            Email
          ...
          <label className="block text-sm font-bold">
            Photo URL <span className="font-normal text-[var(--muted)]">(optional)</span>
            <input autoComplete="url" className={inputClass} name="photoURL" placeholder="https://example.com/photo.jpg" type="url" />
          </label>
          <label className="block text-sm font-bold">
            Password
            <input
              ...
              onChange={(e) => { setPasswordTouched(true); setPasswordError(getPasswordError(e.target.value)); }}
            />
        ...
        <p className="mt-6 text-center text-sm text-[var(--muted)]">
          ...
          <Link className="font-bold text-[var(--accent)]" href="/login">Login</Link>
        </p>
      </div>
    </section>
  );
